...
const baseUrl =
  process.env.NODE_ENV === 'development'
    ? `${process.env.LOCALHOST_FRONTEND_DEPLOY_URL}`
...
const send = async ({ to, firstName, verificationToken }) => {
  let body = `<div style="background-color: #F5F5F5; font-family: 'Inter', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; font-size: 16px; line-height: 1.5; color: #333333; margin: 0; padding: 50px;">
      <div style="text-align: center;"><a href="${baseUrl}" style="display: inline-block; font-size: 3em; font-weight: bold; color: #007BFF; text-decoration: none;">Wallet App</a></div>
      <h2 style="font-size: 1.5em; margin-top: 0; margin-bottom: 0.5em;">Dear ${firstName ?? 'User'},</h2>
      <p style="margin-top: 0;">Thank you for registering in our <a href="${baseUrl}" style="color: #007BFF; text-decoration: none;">Wallet App</a>.</p>
      <div style="text-align: center; margin-top: 2em;"><a href="${baseUrl}${verifyEndpoint}${verificationToken}" style="display: inline-block; padding: 1em 2em; font-size: 1em; font-weight: bold; color: #FFFFFF; text-decoration: none; background-color: #24cca7; border: none; border-radius: 0.5em; cursor: pointer;">Confirm your e-mail</a></div>
